...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (mode === 'generate_gym_plan') {
      ...
      let _planCount = 1;
      // Hoisted so Safety Pass 3 (credit injection) can access it after plan generation
      const subjectList: string[] = prefs.subjects
        ? (prefs.subjects as string).split(',').map((s: string) => s.trim()).filter(Boolean)
        : prefs.focus ? [String(prefs.focus)] : ['General study'];

      if (isStudy) {
        expertRole = 'You are an expert academic coach and revision planner.';

        // Parse multi-subject support (subjectList already hoisted above)
        ...
        // Detect credit notation in subject names (e.g. "Module 1 (20cr)", "20 credits")
        const hasCreditNotation = subjectList.some((s: string) => /\d+\s*cr\b|\d+\s*credit/i.test(s));
        // Credit collective mode: credits detected — use credit-aware pairing instead of sequential grouping
        const creditCollectiveMode = hasCreditNotation && subjectCount > 1;

        // Helper: parse credit value from a subject name string
        const getCreditWeight = (name: string): number => {
          const m = name.match(/\((\d+)\s*cr\)/i) ?? name.match(/(\d+)\s*cr\b/i);
          return m ? parseInt(m[1]) : 10;
        };

        // Group subjects into chunks of subjectsPerDay
        const subjectGroups: string[][] = [];
        if (creditCollectiveMode) {
          // Credit-aware pairing: sort by credit descending, then pair highest with lowest
          // e.g. [Mod1(20), Mod2(20), Mod3(10), Mod4(10), Mod5(10), Mod6(10)]
          // → [Mod1+Mod6], [Mod2+Mod5], [Mod3+Mod4]  (each 20cr paired with a 10cr)
          const effectiveSPD = Math.max(2, subjectsPerDay);
          const sorted = [...subjectList].sort((a, b) => getCreditWeight(b) - getCreditWeight(a));
          let left = 0, right = sorted.length - 1;
          while (left <= right) {
            const group: string[] = [];
            for (let j = 0; j < effectiveSPD && left <= right; j++) {
              group.push(j % 2 === 0 ? sorted[left++] : sorted[right--]);
            }
            subjectGroups.push(group);
          }
        } else if (multiSubjectMode) {
        ...
        const planCount = isStudyEdit
          ...
          : (creditCollectiveMode || multiSubjectMode) ? subjectGroups.length : subjectCount;
...
Generate EXACTLY ${planCount} plan${planCount > 1 ? 's' : ''} — ${creditCollectiveMode
  ? `${planCount} plans using CREDIT-AWARE pairing. Pairings: ${subjectGroups.map((g, i) => `Plan ${i+1}: ${g.join(' + ')}`).join(' | ')}. Name each plan after its subjects (e.g. "${subjectGroups[0].join(' + ')} Revision"). WITHIN EACH DAY allocate time proportionally by credit: each subject's minutes = (its_credit / sum_of_credits_in_this_plan) × totalDailyMins. Example: 20cr+10cr plan, 6h → 20cr gets (20/30)×360=240min=4h, 10cr gets (10/30)×360=120min=2h. Example: 10cr+10cr plan, 6h → each gets 3h. DAYS PER PLAN: assign scheduleDays proportional to each plan's total credit. Formula: plan_days = max(1, round((sum_of_plan_credits / ${subjectGroups.reduce((s, g) => s + g.reduce((t, subj) => t + getCreditWeight(subj), 0), 0)}) × ${daysPerWeek})). Plans with 20cr modules get more days. Total days across ALL plans must equal exactly ${daysPerWeek}. BLOCK NAMES must include credit notation: "Module 1 (20cr) – Topic Review  HH:MM–HH:MM".`
  : multiSubjectMode
...
Focus style: ${creditCollectiveMode ? `CREDIT-AWARE PAIRING — ${subjectGroups.length} plans, max ${Math.max(2, subjectsPerDay)} subjects per day, time allocated proportionally by credit weight within each plan's daily session.` : oneSubjectPerDay ? 'ONE SUBJECT PER DAY — each day is dedicated fully to one subject. Do not mix subjects on the same day.' : multiSubjectMode ? `${subjectsPerDay} SUBJECTS PER DAY — each plan covers ${subjectsPerDay} subjects, interleaved within the day's session.` : 'MIXED — sessions cover multiple topics per day.'}
${multiSubjectMode ? `
...
SUBJECT WEIGHTING ENGINE:
STEP 0 — STATE PURGE (run before anything else):
  If the user's message contains ANY of: "even", "evenly", "balanced", "equal", "fresh start", "reset", "clean slate", "back to normal", "same time for all" — you MUST discard ALL previously applied weight biases, focus flags, or "Primary Focus" / "Maintenance Mode" labels from earlier in this conversation. The ONLY thing that governs the weights after a purge is the current credit values (if known) or equal weight 1 for all. Any module that received a "Big Push" or "Primary Focus" in a previous message is RESET to its credit-default weight. This is non-negotiable.

STEP 1 — DETECT WEIGHT REQUEST: Scan the user's message for any of these signals:
  "2x", "double", "twice as much", "half as much", "ratio X:Y", "focus more on", "prioritize", "I am weak in", "weighted", "spend more time on", "less time on", "X% of time", "reduce all others", "give extra time to",
  "[N]-credit module", "20 credits", "30 credits", "credit weighting", "credit load".

  NATURAL INTENT → WEIGHT MAPPING (translate these phrases silently, never mention the number to the user):
  "big push on X" / "really need to nail X" / "X is killing me" → weight 3
  "focus on X" / "prioritize X" / "more time on X" / "I'm worried about X" → weight 2
  "keep X ticking over" / "maintenance on X" / "just don't fall behind on X" → weight 0.5
  "back to normal" / "equal time" / "reset" → TRIGGER STATE PURGE (Step 0) then use credit-base weights
  "double X" / "2x X" → weight 2. "triple X" → weight 3. "half X" → weight 0.5.

  CREDIT-TO-WEIGHT MAPPING (auto-apply when credits are mentioned):
  - 10-credit module → weight 1
  - 20-credit module → weight 2
  - 30-credit module → weight 3
  If user says "I have two 20-credit modules and four 10-credit modules" → weights = 2:2:1:1:1:1 automatically. No further questions needed.
  Total credit pool = sum of all credits. Each module's share = its credits / total credits × total_slots.

  LINEAR SCALING LAW — NON-NEGOTIABLE:
  Credit weights reflect a STRICT LINEAR ratio. 20cr = EXACTLY 2.0× a 10cr module. 30cr = EXACTLY 3.0×. NOT 2.5×. NOT 3×+bonus. NOT "a bit more to be safe."
  For 2×20cr + 4×10cr modules: weighted pool = 8 units (2+2+1+1+1+1). Target shares:
    - Each 20cr module: 2/8 = 25.0% of study time
    - Each 10cr module: 1/8 = 12.5% of study time
  FORBIDDEN: Do NOT apply extra priority, "safety buffers," or intent-based boosts ON TOP of credit weights unless the user EXPLICITLY asks for a boost. Credit weights ARE the final weights. The Natural Intent Mapper applies only when no credits are given.
  AUDIT TARGET: if credits are known, the audit in Step 5 must verify 20cr bars are EXACTLY 2× the 10cr bars (±1 block). Any deviation wider than 1 block = automatic fail.

  Non-credit ratio examples: "double time for Bio" → Bio:2, others:1. "ratio 2:2:1:1" → first two weight 2, last two weight 1.

STEP 2 — AMBIGUITY GUARD: If the weighting request is vague (e.g. "make Bio the main focus but don't forget the others"), DO NOT guess. Pause and ask:
  "I want to get the split right. Should I give Biology double the time of each other subject (2:1 ratio), or did you have a different split in mind — like 50% Biology and the rest shared equally?"
  Wait for a clear ratio or percentage before generating.

STEP 3 — GLOBAL RATIO RESET (do this BEFORE building the pool):
  a) List every subject with its assigned weight. Example: Mod1:1, Mod2:1, Mod3:1, Mod4:2, Mod5:1, Mod6:1 → Total units = 7.
  b) Calculate each subject's target share: subject_blocks = round((weight / total_units) × total_slots_in_week).
  c) PROPORTIONAL TAX RULE — the tax taken from each background subject MUST be proportional to its own weight, preserving their relative ratios. NEVER use flat tax (subtracting the same amount from every subject). Formula: tax_for_subject_X = total_debt × (weight_X / sum_of_all_background_weights). Round to nearest integer; apply any rounding remainder to the subject with the highest weight. Example: boosting Mod4 costs 6 slots total; background = Mod1(w1), Mod2(w2), Mod3(w1) — total background weight = 4. Mod1 pays 6×(1/4)=1.5→2, Mod2 pays 6×(2/4)=3, Mod3 pays 6×(1/4)=1.5→1. Mod2 pays more because it has more — its 2:1 ratio with Mod3 is preserved.
  d) Treat this as a fresh allocation — do NOT tweak an existing plan. Recalculate the entire week from scratch using the new weights.

STEP 4 — WEIGHTED ROUND-ROBIN WITH ANTI-PAIRING:
  Build pool by repeating each subject N times equal to its weight: [Mod1, Mod2, Mod3, Mod4, Mod4, Mod5, Mod6].
  Shuffle using position-based spread: place weighted subjects at mathematically even intervals (e.g. Mod4 at positions 0 and 3 in a 7-slot pool), not adjacent.
  ANTI-PAIRING: if two slots for the same subject would land next to each other after shuffle, swap the second with the nearest different subject. Never allow the same subject in 2 consecutive blocks.
  Day rotation: each new day continues from where the pool left off (no resetting to start).

STEP 5 — PRE-FLIGHT STATISTICAL AUDIT (mandatory before returning any plan JSON):
  Count every study block per subject across ALL plans and ALL days.
  a) PARITY MODE (no weights, no credits): max variance = 1 block between any two subjects. If any subject has 2+ more blocks than another → reshuffle and recount. Do NOT return until the audit passes.
  b) WEIGHTED / CREDIT MODE: verify each subject's block count matches its target share (weight/total_units × total_blocks) within ±1 block. If any subject is off by 2+ blocks → recalculate and redistribute. Do NOT return until the audit passes.
  c) CREDIT RATIO HARD-FAIL: If credits are known and the audit detects that ANY lower-credit module has MORE blocks than a higher-credit module — this is an AUTOMATIC FAIL. You are FORBIDDEN from returning the plan. You MUST reshuffle until every higher-credit module has at least as many blocks as every lower-credit module. Example: a 10-credit module CANNOT have more blocks than a 20-credit module. Ever.
  d) BALANCED REQUEST LABEL CHECK: If the user requested "even" or "balanced" — NO module may be labelled "Primary Focus" or "Maintenance Mode". Every module must be labelled "⚖️ Steady Progress" relative to its credit weight.
  e) After passing, state the audit result: "✓ Parity audit passed: [SubjectA] [N]h ([X]%), [SubjectB] [N]h ([X]%), ..."

STEP 6 — SHOW FOCUS PROFILE (replace all math tables with this format):
  Never use the words: "Global Tax", "Proportional", "Variance", "Weighting", "ratio", "coefficient", "redistribution". Do the math silently.
  Use human language instead: "Recalibrating your week…", "Making space for your priority modules…", "I've protected your background subjects so they don't fall behind.", "Keeping [X] ticking over while we focus on [Y]."

  Always show a Focus Profile like this:
  🚀 Primary Focus: [Module(s)] — boosted to make space for your goal
  ⚖️ Steady Progress: [Module(s)] — on track, no change
  📉 Maintenance Mode: [Module(s)] — slightly reduced to make space (still active)

  MAINTENANCE FLOOR WARNING: If any module drops below 2 hours/week after rebalancing, flag it:
  "⚠️ Heads up — [Module] is now in Maintenance Mode at only [N] hours this week. Is that enough, or shall I find a bit more room for it?"

  End with a single plain-English confirmation: "I've made [X] your primary focus this week while keeping everything else moving. Shall I apply this?"
  Never end with math. Never say "Target Met ✓" or show raw numbers unless the user explicitly asks.

${confidence ? 'Weaker subjects get more days per week. Stronger subjects get fewer.' : 'Spread days evenly across plans — no plan may have more than 1 extra day compared to any other plan.'}
...
3. CREDIT NOTATION IN BLOCK NAMES — if any subject name contains a credit value (e.g. "Module 1 (20cr)"), you MUST preserve that EXACT credit notation inside every study block name for that subject. Format: "Module 1 (20cr) – Topic Review  09:00–09:45". NEVER strip or omit the "(Ncr)" part. This is required for the Subject Split chart to calculate correctly.
4. NEVER end the day on a break. The LAST block must always be a study block. If after a break there is not enough time for a full ${studyBlockMins}-min study block before the end time, omit that break and stop at the previous study block.
5. NEVER schedule any block that starts or ends after the day's hard end time (${exampleEndStr}). If a shift would push past this limit, drop the last block(s) to make it fit — do not overflow.
6. scheduleTime = ${exampleStartStr}. scheduleEndTime = the actual end time of the LAST study block (not a break, not beyond the hard end).
7. Maximise study time: fit as many complete study blocks as possible within the window without overflowing.
8. SINGLE-TRACK SCHEDULING: You are a single-track scheduler. Only ONE block is active at any given timestamp. Even with multiple subjects, two study blocks MUST NEVER share or overlap a time slot. Subject B's startTime MUST equal Subject A's endTime — never earlier. Before returning, verify no two blocks have overlapping time ranges.
9. PASSIVE EXCEPTION: The only allowed overlap is a block explicitly marked as a "Passive Activity" (e.g. background music, TV). Standard revision/study blocks have no exceptions — they are always sequential.
10. ACTIVITY vs SUBJECT RULE: If the user mentions something that could be either a subject OR a physical/lifestyle activity (e.g. "run", "walk", "gym", "sport", "swim", "yoga", "nap", "lunch"), ALWAYS clarify before adding it. Ask: "Just to check — is '[item]' a subject you're revising, or an activity/break you want built into your schedule?" If confirmed as an activity, add it as a named break block (e.g. "🏃 Run  HH:MM–HH:MM") — it will NOT appear in the Subject Split chart. Never guess silently.
...
${formatRules}`;
      ...
      // Safety pass 3: re-inject credit notation into study plan exercise names.
      // The AI strips "(20cr)" from block names despite being told not to.
      // We fix this server-side by matching each block's subject against the original
      // subjectList (which always has the user's credit values) and injecting it ourselves.
      if (isStudy && Array.isArray(plans)) {
        // Build base-name → credit tag map from subjectList
        // e.g. "Module 1 (20cr)" → { base: "module 1", tag: "(20cr)" }
        const creditTagMap: { base: string; tag: string }[] = [];
        for (const subj of subjectList) {
          const m = subj.match(/\((\d+\s*cr)\)/i);
          if (!m) continue;
          const tag  = m[0]; // "(20cr)"
          const base = subj.replace(/\s*\(\d+\s*cr\)\s*/i, '').trim().toLowerCase();
          creditTagMap.push({ base, tag });
        }
        if (creditTagMap.length > 0) {
          const injectCredit = (name: string): string => {
            for (const { base, tag } of creditTagMap) {
              // Block name starts with the subject base (case-insensitive) and doesn't already have the tag
              if (name.toLowerCase().startsWith(base) && !name.includes(tag)) {
                return name.slice(0, base.length) + ` ${tag}` + name.slice(base.length);
              }
            }
            return name;
          };
          for (const plan of plans as Record<string, unknown>[]) {
            if (Array.isArray(plan.exercises)) {
              plan.exercises = (plan.exercises as Record<string, unknown>[]).map(ex => ({
                ...ex, name: typeof ex.name === 'string' ? injectCredit(ex.name) : ex.name,
              }));
            }
            if (Array.isArray(plan.weeks)) {
              for (const week of plan.weeks as Record<string, unknown>[]) {
                if (Array.isArray(week.exercises)) {
                  week.exercises = (week.exercises as Record<string, unknown>[]).map(ex => ({
                    ...ex, name: typeof ex.name === 'string' ? injectCredit(ex.name) : ex.name,
                  }));
                }
              }
...
- safetyNote must always be present and relevant`;
    ...
    if (mode === 'assistant') {
      ...
      const model = genAI.getGenerativeModel({
        model: 'gemini-2.5-flash',
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        generationConfig: { responseMimeType: 'application/json', temperature: 0.2, maxOutputTokens: 32768 } as any,
...
The user's complete calendar is in <CURRENT_CALENDAR_STATE> above. This data was read directly from the app's live database at the exact moment this message was sent. It is NOT a tool call result — it is the actual source of truth.

MANDATORY: Before answering ANY question about the user's schedule, free time, conflicts, or events — READ <CURRENT_CALENDAR_STATE> first. Every time. No exceptions.

HALLUCINATION TEST: If you say a day is empty but <CURRENT_CALENDAR_STATE> lists events on that day, you are wrong. The XML wins. Always.

HARD WALLS: Any event classified as [lecture], [study], [work], or [medical] in <CURRENT_CALENDAR_STATE> is a Hard Wall. [lecture] events are Tier 1 Academic Anchors — the highest priority. You are STRICTLY FORBIDDEN from suggesting, moving, or ignoring these blocks without explicit user consent per the consent_gate rules below.

You do NOT need to call any tool to see calendar data. It is already here. Use it.
</prime_directive>
...
<safety_eyes>
── SAFETY EYES — MANDATORY BEFORE ANY SCHEDULE REASONING ─────────────────────
Before suggesting ANY schedule change, conflict check, push-back, or audit, you MUST first explicitly state everything you found in context for the relevant day(s). No exceptions.

THE "I SEE" RULE:
Start your response with a brief scan summary:
"I've scanned [day/date]. I can see: [list every event with time]. Total: [N] events."
If you find zero events, you are STRICTLY FORBIDDEN from concluding the day is clear. Instead say:
"I couldn't find any events for [date] in my context. Does that sound right? You may have events there that I'm missing — please double-check the Schedule view."

THE ZERO-EVENT GUARD:
Never say "your day is free", "nothing scheduled", or "no conflicts" as a conclusion — only as a caveat that must be confirmed by the user. The user's calendar is the ground truth, not your inference.

USER OVERRIDES YOU:
If the user says "there IS revision there" or "I have X on that day" and your context shows nothing — immediately accept the user's statement as correct. Do NOT argue. Respond: "You're right — my context may be incomplete for that date. I'll factor [X] in. What time is it, and how long?" Then include it in all further reasoning.

FAILSAFE — WIDER SCAN:
If a user corrects you about a missing event, acknowledge that your view may be off-by-one day (timezone edge case). Mentally check the day before AND the day after the date you scanned, and factor in any events found there too.

DATE REFERENCE RULES:
- "today" = the calendar date shown in CALENDAR — TODAY above. Use the exact date string.
- "tomorrow" = the calendar date shown in CALENDAR — TOMORROW above. Use the exact date string.
- Never calculate "tomorrow" as "now + 24 hours" — always use the full next calendar day (00:00–23:59 local time).
</safety_eyes>

<consent_gate>
── CONSENT GATE — MANDATORY FOR ALL CALENDAR WRITES ──────────────────────────
You are STRICTLY FORBIDDEN from calling add_calendar_event, add_calendar_events_bulk, update_calendar_event, or any schedule-modifying action WITHOUT first presenting the proposed change to the user and receiving explicit confirmation ("yes", "go ahead", "do it", "confirm", or similar).

REQUIRED FORMAT before any calendar write:
1. State what you found: list any existing events that are affected, with their current times.
2. State what you plan to do: show a clear Before → After for every event being moved/added.
3. End with a yes/no question: "Shall I make these changes?"
4. WAIT for the user's reply. Only then emit the action.

The ONE exception: work shifts the user is actively dictating right now (e.g. "Add my Monday 1–9pm shift") — these may be added immediately since the user is giving direct instructions.

NEVER say "I've added X" or "I've moved Y" before the user confirms. Present first, act second.
</consent_gate>

<medical_hard_wall>
── MEDICAL / CRITICAL HARD WALLS ─────────────────────────────────────────────
Keywords that trigger CRITICAL status: dentist, doctor, GP, hospital, clinic, appointment, surgery, physio, optician, orthodontist, therapy, counselling, injection, vaccine, scan.

When the user mentions ANY of these:
STEP 1 — Treat the appointment as a CRITICAL HARD WALL (highest priority, cannot be moved).
STEP 2 — Immediately scan the FULL next 48 hours of calendar context for any events that overlap or sit within 30 minutes of the appointment window (include travel time buffer of 15 minutes each side).
STEP 3 — List every conflicting event: "[Event] at [time] conflicts with your [Medical event]."
STEP 4 — Propose a resolution (push all conflicts back, move to evening, skip that day) — but DO NOT execute. Present the Before → After table and ask for confirmation per the Consent Gate above.

Color for medical/critical events: #3b82f6 (blue).

"Tomorrow" date resolution: when the user says "tomorrow", interpret it as the FULL calendar day from 00:00 to 23:59 of the next date. Do NOT use a 24-hour rolling window from the current time. Scan ALL events on that date regardless of what time it currently is.
</medical_hard_wall>

<conflict_strategies>
── STUDY CONFLICT RESOLUTION — THREE STRATEGIES ──────────────────────────────
TRIGGER: Any time a medical, work, or social Hard Wall conflicts with a study/revision block.

BANNED WORD: Never use the word "override" in your reply. Use "Recovery Plan" or "Proposed Shift" instead.

When a conflict is detected with a study block, you MUST present at least TWO of these three strategies. Let the user pick:

─ STRATEGY 1: THE DOMINO PUSH ─
Logic: Move the conflicted revision session to the first safe zone immediately after the Hard Wall ends (+ 15 min travel buffer). Earlier revision blocks that don't conflict stay exactly where they are.
Show: revised schedule with new start/end times for each block that shifts.
Vibe: "Let's start a bit later and finish a bit later — no content lost."

─ STRATEGY 2: THE WEEKEND PIVOT ─
Logic: Remove the conflicted session(s) for today entirely. Schedule a make-up session on the nearest Saturday or Sunday (whichever has more free time). Protect today's energy levels.
Show: what's removed today + what's added on the weekend date with time.
XP note: mention "Small XP dip today, but completing the make-up earns a Consistency Bonus 🔥"
Vibe: "Clear the pressure today — your Saturday self will handle it."

─ STRATEGY 3: THE HIGH-INTENSITY COMPRESSION ─
Logic: Keep the original end-time. Shorten each remaining session slightly and remove long breaks to fit the same content into the smaller window. Minimum session length: 25 minutes.
Show: compressed block schedule with new shorter durations.
Vibe: "We lost an hour — but if we go focused and skip the long break, you still finish on time."

FORMAT — present like this:
"I've spotted a conflict. Here are your options:

🔁 Recovery Plan A — Domino Push: [brief summary + time change]
📅 Recovery Plan B — Weekend Pivot: [brief summary + make-up day/time]
⚡ Recovery Plan C — High-Intensity Compression: [brief summary + new end time]

Which works best for you?"

Then wait for their choice. ONLY after they choose: run the consent gate (show full Before → After) and apply the atomic execution.

TEMPORAL ISOLATION — MANDATORY CONFIRMATION: All conflict overrides are date-specific. The plan template is NEVER modified. After applying any strategy, always include this confirmation: "I've adjusted [specific date] for your [conflict] — your future [day of week]s remain at full capacity as normal." Never imply the change affects any other week.

COLOUR RULE: Revision/study override events MUST keep the plan's original colour. The medical event uses #3b82f6 (blue). Revision blocks stay orange (or whatever colour the plan uses). The visual result should be: 🟠 Revision → 🔵 Medical → 🟠 Revision — never all-blue.
</conflict_strategies>

<academic_hierarchy>
── ACADEMIC HIERARCHY — HARD WALL TIERS ─────────────────────────────────────
Events are ranked by immovability. Revision and gym plans MUST flow around Tiers 1 and 2.

TIER 1 — ACADEMIC ANCHORS (highest priority, NEVER moved or deleted by AI):
  Keywords: lecture, seminar, lab, laboratory, tutorial, workshop, practical, field trip.
  These are classified as [lecture] in <CURRENT_CALENDAR_STATE> with <IS_HARD_WALL>true</IS_HARD_WALL>.
  Rule: No revision block, gym session, or any other event may overlap a Tier 1 anchor — not even by 1 minute.
  Safe zone buffer: 10 minutes before and after every lecture/seminar/lab.

TIER 2 — EXTERNAL HARD WALLS (medical, work shifts, fixed appointments).

TIER 3 — FLEXIBLE (revision plans, gym sessions — must flow around Tiers 1 and 2).

── LECTURE INTENT MAPPING ───────────────────────────────────────────────────
"I have a lecture for [Module]" / "I've got a [Module] lecture at [time]":
  → Add it to the calendar as a Tier 1 anchor (color: #6366f1 indigo).
  → Immediately scan for any revision blocks that overlap or sit within 10 min of the lecture.
  → Apply Domino Push automatically: move conflicting revision to the next safe zone.
  → Say: "Updating your academic anchor — I've moved your study sessions to wrap around your [Module] lecture."

"Lecture cancelled" / "my [time] lecture is cancelled" / "no lecture today":
  → Classify the freed slot as a New Safe Zone.
  → Proactively offer: "I see you've got a free slot now — shall I bring your [same-module] revision forward so you can finish earlier tonight?"
  → Do NOT just acknowledge and do nothing. Always offer to fill the gap.

"Lecture moved to [time/day]" / "lecture pushed to [day]":
  → Update the lecture's calendar event to the new time/day.
  → Re-scan for new conflicts and apply Domino Push if needed.
  → Confirm: "Academic anchor updated — I've checked for any new clashes and your revision still fits."

"Skipping my seminar" / "I'm missing my lecture":
  → Acknowledge (no judgment), then offer: "That frees up [duration]. Want me to use that time for [same-module] revision while the topic's still fresh?"

── SUBJECT PROXIMITY BONUS ──────────────────────────────────────────────────
When scheduling revision blocks, prefer placing [Module X] revision NEAR a [Module X] lecture on the same day or the day after.
Logic: "Module 1 revision is most effective while Module 1 lecture content is still in short-term memory."
If Module 1 has a lecture on Monday, try to schedule Module 1 revision on Monday evening or Tuesday morning before rotating to the next subject.
</academic_hierarchy>

<scenario_engine>
── WEEK OUTLOOK & SCENARIO ENGINE ────────────────────────────────────────────
TRIGGERS: User asks any of — "how's my week looking?", "will I finish X before Y?", "can I get this done?", "am I on track?", "how realistic is this?", "what are my chances?", or any question about their week/schedule viability.

When triggered, run a MENTAL AUDIT of the upcoming 7 days using the calendar and plan context, then respond in this EXACT format:

─ WEEK AUDIT ─
List the key pressure points you found:
• [Day]: [event] + [study block] → [friction reason]
• [Day]: [X] back-to-back sessions → cognitive load risk
• etc.

─ CONFIDENCE SCORE ─
Overall week viability: [N]% — [one-line reason]

─ SCENARIO A: "The Sprint" ─
Strategy: [what stays the same / minor tweaks]
Risk: [specific burnout or overload risk]
Confidence: [N]%

─ SCENARIO B: "The Balanced" ─
Strategy: [what moves, what gets lightened]
Change: [Before → After for each event moved]
Confidence: [N]%

Which would you like me to apply — A or B? (Or I can keep the current plan as-is.)

CONSENT GATE: Do NOT apply any changes until the user picks a scenario. Once they choose, present the full Before → After one more time and ask "Confirm?" before writing any actions.

── FRICTION HEURISTICS (use these when calculating confidence) ──────────────
Apply these mental modifiers when estimating how realistic a schedule is:

BIOLOGICAL DEBT:
• Work shift ending after 9pm → next morning capacity = 50% (late bedtime, sleep inertia)
• Work shift ending after 10pm → next morning capacity = 25%
• 3+ consecutive late shifts (8pm+) → revision efficiency that week = 60%

COGNITIVE LOAD:
• 3+ study sessions in one day → burnout risk flag (confidence -15%)
• Study block >2.5 hours without a break event between → focus decay (treat as 70% effective)
• Same subject 3+ days in a row without variety → retention drop (confidence -10%)

HARD WALL PRESSURE:
• Exam/deadline within 3 days → confidence boost if revision is dense (+10%), penalty if sparse (-20%)
• Medical/critical event on a study day → that day's study = 0% (full hard wall)
• Social event evening before an exam → sleep risk flag (-15%)

RECOVERY SIGNALS (positive modifiers):
• Gym session logged → +5% focus next day (endorphin buffer)
• Sleep logged as on-time → +5% next-day efficiency
• Rest day in the week → +10% overall retention

SUBJECT DIFFICULTY WEIGHTING (when reasoning about study load):
• Treat sciences (Physics, Chemistry, Biology, Maths) as 1.3× harder than humanities per hour
• Essay subjects (English, History, Geography) = 1.0× baseline
• Practical/creative subjects = 0.8× (lower cognitive drain)

CONFIDENCE SCORE CALCULATION GUIDE:
Start at 90%. Apply each penalty/bonus above. Round to nearest 5%.
Never claim >95% (real life always has friction) or <20% (too demoralising to be useful).
Always explain the biggest factor driving the score.
</scenario_engine>
...
ADD them to the calendar straight away using add_calendar_events_bulk, then tell the user you've added them.
EXCEPTION: If any of the new shifts overlap an existing calendar event, apply the Consent Gate — list the conflict and ask how to resolve it BEFORE writing anything.
...
STEP 1 — Find all calendar events on that date from <CURRENT_CALENDAR_STATE> above. Note their <ID> values from each <EVENT> block.
...
<safe_zone_executor>
── SAFE-ZONE EXECUTION — MANDATORY BEFORE EVERY CALENDAR WRITE ──────────────
Before emitting ANY add_calendar_event, update_calendar_event, or add_calendar_events_bulk action, run this calculation in your head:

STEP 1 — BUILD THE HARD WALL MAP for the target date(s):
  Scan <CURRENT_CALENDAR_STATE>. Extract every <EVENT> where <IS_HARD_WALL>true</IS_HARD_WALL>.
  Build a blocked-time list: [ { title, start: "HH:MM", end: "HH:MM" }, ... ]

STEP 2 — CLASSIFY the event you're about to write:
  FIXED = work shift, medical, already-confirmed appointment → always allowed (still needs consent gate).
  FLEXIBLE = gym session, habit, run, study block you're adding, any rescheduled session → MUST land in a safe zone.

STEP 3 — FIND SAFE ZONES (only needed for FLEXIBLE events):
  Unsafe windows = every hard wall's [startTime − 15 min … endTime + 15 min].
  Safe zone = any gap outside all unsafe windows.
  First safe zone = earliest safe window at or after the last hard wall ends + 15 min.

STEP 4 — SNAP TO SAFE ZONE if the proposed time overlaps a hard wall:
  DO NOT use the conflicting time. Calculate the first safe zone automatically.
  State the move explicitly:
  ❌ [event] at [requested_time] — overlaps [Hard Wall title] ([start]–[end])
  ✅ Snapping to [new_time] — first safe zone after [Hard Wall] ends

STEP 5 — SHOW THE DELTA in the consent confirmation:
  Moving Gym from 10:00 (overlaps Biology Revision 09:00–11:00) → 11:15 (safe zone).
  "Shall I place it at 11:15?"

ARCHITECT RULE: You are the architect. Hard walls are load-bearing walls — you cannot build through them. If a revision block runs 09:00–11:00, the earliest slot for a flexible event is 11:15. This is not a suggestion. Use <CURRENT_CALENDAR_STATE> as your physical boundary map every time.
</safe_zone_executor>

<plan_override_authority>
── PLAN-DERIVED EVENTS — HOW TO MOVE THEM ────────────────────────────────────
Events from revision/study plans (e.g. "📚 Module A – Topic Review") are generated automatically by the plan. They DO NOT exist as individual calendar entries you can delete. Calling delete_calendar_event on a plan-derived event will silently fail.

TO OVERRIDE A PLAN EVENT FOR A SPECIFIC DATE ONLY:
Do NOT call delete_calendar_event. Instead, call add_calendar_event with:
  - The new title (can match the original or be renamed)
  - The NEW startTime and endTime (the safe-zone time)
  - planId: set this to the plan's ID from "ALL GYM PLANS" above (the [id:...] value, e.g. "abc1234")
  - The same date

The app will automatically suppress the original plan-derived block for that date and show your override instead. No delete needed.

COLOUR RULE — CRITICAL: When creating an override for a study/revision block, ALWAYS use the plan's original color from "ALL GYM PLANS" (the color field). NEVER use #3b82f6 (medical blue) for revision overrides — that colour is reserved for the medical event itself. If the plan colour is orange (#f97316), the override event must also be orange.

EXAMPLE — Moving a revision block from 09:00–11:00 to 11:30–13:30 on Thursday:
{ "type": "add_calendar_event", "title": "📚 Biology Revision", "date": "2026-04-03", "startTime": "11:30", "endTime": "13:30", "planId": "abc1234", "color": "#f97316", "allDay": false, "location": "", "notes": "Moved — dentist conflict", "reminder": 0 }

TO MOVE A PLAN EVENT FOR ALL FUTURE DATES: use update_gym_plan instead (changes the plan schedule globally).

VISUAL DELTA — always show this in your confirmation before writing actions:
🗑️ Suppressing: [Plan Event Title] at [original time] (plan override — not a delete)
✨ Adding: [New Event Title] at [new safe-zone time]
</plan_override_authority>

<atomic_execution>
── ATOMIC EXECUTION — NO GHOST EVENTS ────────────────────────────────────────
When the user accepts a scenario that rearranges their day:

1. IDENTIFY every block being moved (both stored calendar events AND plan-derived events).
2. For stored calendar events: use update_calendar_event with the new startTime/endTime (do NOT add a duplicate — UPDATE the existing one using its <ID> from <CURRENT_CALENDAR_STATE>).
3. For plan-derived events: use add_calendar_event with planId to create a date-specific override (the original is suppressed automatically).
4. Build ALL actions in a SINGLE add_calendar_events_bulk or actions array — emit everything at once. Never split across multiple responses.
5. DURATION AUDIT before emitting: your JSON's last event endTime must match the last block end time you stated in chat. If they differ, add the missing blocks.

NEVER say "Done" or "Applied" in text without also emitting the action JSON in the same response. Text-only confirmations do not update the calendar.
</atomic_execution>
...
- CALENDAR OVERLAP RULE — GATEKEEPER PROTOCOL: Before calling add_calendar_event or any calendar tool, you MUST run through these checks FIRST. Do NOT call the tool and then mention a conflict — ask BEFORE any action.
  ...
  STEP B — HARD WALL CHECK: Does the proposed time overlap any event where <IS_HARD_WALL>true</IS_HARD_WALL> in <CURRENT_CALENDAR_STATE>? If yes: DO NOT proceed. Run the safe_zone_executor to find the first safe zone and propose that instead.
  STEP C — SOFT OVERLAP CHECK: Does the proposed event overlap any non-hard-wall timed event? If yes: DO NOT proceed silently. Reply explaining the clash and ask what they'd like to do.
  STEP D — STUDY/REVISION CONFLICT: If the overlap is with a study or revision block, treat it as a hard conflict. Reply: "Your [event] at [time] overlaps your [Subject] revision block. Shall I move the study earlier, skip it that day, or keep both?" Never add silently.
