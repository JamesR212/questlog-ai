...
function extractSubject(blockName: string): string {
  // Split on en-dash (–) or " - " (spaced hyphen) first, then strip trailing time patterns
  const part = blockName.split(/\s*–\s*|\s+-\s+/)[0];
  return part.replace(/\s+\d{1,2}:\d{2}.*$/, '').trim();
}

function isBreakBlock(name: string): boolean {
  // Use word boundaries to avoid false positives like "run through", "walking route", etc.
  return /☕|🍽|🏋|🏃|🚶|🧘|🚴|🏊|\b(break|lunch|gym|run|walk|jog|exercise|sport|swim|cycle|yoga|stretch|nap|rest)\b/i.test(name);
}
...
// Parse credit value from a subject name, e.g. "Module 1 (20cr)" → 20
function parseCreditValue(subject: string): number | null {
  const m = subject.match(/\((\d+)\s*cr\)/i) ?? subject.match(/(\d+)\s*cr\b/i);
...
export default function StudyInsights() {
  ...
  for (const plan of studyPlans) {
    ...
    for (const ex of allExercises(plan)) {
      ...
      // allExercises() returns one session template per week — multiply by scheduleDays.length
      // so a plan running 2 days/week contributes 2× more than one running 1 day/week
      const daysPerWeek = Math.max(1, plan.scheduleDays.length);
      s.blocks += daysPerWeek;
      s.totalMins += mins * daysPerWeek;
      ...
      s.doneMins += Math.round(mins * daysPerWeek * doneRatio);
  ...
  const totalMinsAll = entries.reduce((s, [, v]) => s + v.totalMins, 0);

  // If every subject has a credit value in its name (e.g. "Module 1 (20cr)"),
  // use credit-weighted percentages directly — this is always correct regardless
  // of how many plans the AI created or how it split the days.
  const creditValues = entries.map(([subject]) => parseCreditValue(subject));
  const allHaveCredits = creditValues.every(c => c !== null);
  const totalCredits   = allHaveCredits ? creditValues.reduce((s, c) => s + c!, 0) : 0;

  const subjects: SubjectStats[] = entries
    .sort((a, b) => {
      if (allHaveCredits) {
        const ca = parseCreditValue(a[0]) ?? 0;
        const cb = parseCreditValue(b[0]) ?? 0;
        return cb - ca; // sort by credit weight descending
      }
      return b[1].totalMins - a[1].totalMins;
    })
    .map(([subject, v], i) => {
      const credit = parseCreditValue(subject);
      const pct = allHaveCredits && credit !== null && totalCredits > 0
        ? Math.round((credit / totalCredits) * 100)
        : totalMinsAll > 0
          ? Math.round((v.totalMins / totalMinsAll) * 100)
          : Math.round((v.blocks / totalBlocks) * 100);
      return {
        subject,
    ...
    <>
      <div className="rounded-2xl border border-ql bg-ql-surface p-4 flex flex-col gap-3">
        <div className="flex items-center justify-between">
          ...
          <p className="text-ql-3 text-[10px]">{studyPlans.length} plan{studyPlans.length > 1 ? 's' : ''} · {totalMinsAll > 0 ? `${Math.round(totalMinsAll / 60)}h total` : `${totalBlocks} blocks`}</p>
        </div>
