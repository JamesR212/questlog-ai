        ...
        {THEME_PHONES.map((t, i) => (
          <div
            ...
            style={{
              ...
              opacity: visible ? 1 : 0,
              transition: `transform 1.5s ${AE} ${delays[i]}ms, opacity 1.0s ${AE} ${delays[i]}ms`,
            }}
          >
            <ThemePhoneMini t={t} />
            <div style={{
  ...
  return (
    <div style={{ background: '#050508', color: '#fff', fontFamily: '-apple-system, BlinkMacSystemFont, "SF Pro Display", "Segoe UI", sans-serif', overflow: 'clip' }}>
      ...
      <section ref={aiRef} style={{ background: '#050508' }}>
        ...
        {/* Sticky two-column */}
        <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', maxWidth: 1020, margin: '0 auto', padding: isMobile ? '0 16px' : '0 48px', boxSizing: 'border-box' as const }}>

          {/* Left: scrolling panels */}
          <div>
            {/* Panel 1 — AI Gym Plans */}
            <div style={{ minHeight: isMobile ? 'auto' : '100vh', display: 'flex', flexDirection: 'column', justifyContent: 'center', paddingRight: isMobile ? 0 : 48, paddingBottom: isMobile ? 48 : 0 }}>
          ...
          {/* Right: sticky image */}
          <div style={{ position: isMobile ? 'relative' : 'sticky', top: 0, height: isMobile ? 'auto' : '100vh', order: isMobile ? -1 : 0, alignSelf: 'start', display: 'flex', alignItems: 'center', justifyContent: 'center', paddingLeft: isMobile ? 0 : 48, paddingBottom: isMobile ? 32 : 0 }}>
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img
              src="/gym-plan.jpg"
              alt="AI-generated gym plan with muscular load radar chart"
              style={{ maxHeight: isMobile ? '60vh' : '76vh', width: '100%', maxWidth: isMobile ? '100%' : 'none', borderRadius: 20, display: 'block', objectFit: 'contain' as const }}
            />
          </div>
