  ...
  return (
    <svg viewBox="0 0 200 215" className="w-full max-w-xs mx-auto">
      {[0.25, 0.5, 0.75, 1].map(s => (
        <path key={s} d={hexPath(s)} fill="none" stroke="var(--ql-border,#e5e5e5)" strokeWidth="0.8" />
      ))}
      {MUSCLE_KEYS.map(k => {
        const end = toXY(MUSCLE_ANGLES[k], R);
        return <line key={k} x1={cx} y1={cy} x2={end.x} y2={end.y} stroke="var(--ql-border,#e5e5e5)" strokeWidth="0.8" />;
      })}
